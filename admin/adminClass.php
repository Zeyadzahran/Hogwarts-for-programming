<?php
    ...
    class admin extends Dbh{
        ...
        public function addHousePoints($houseId,$points)
        {
            ...
            if (!$stmt->execute([$points, $houseId])) {
                header("location: /professor/manageCourses?error=statementfailed");
                exit();
            }
            return true;
        }
        ...
        public function GetUsers($id)
    {
        ...
        public function addquiz($quizname,$courseid,$points)
        {
            ...
            $quizid= $connection->lastInsertId();
            return $quizid;
        }
        ...
        public function addnewcourse($coursename,$professorname){
            ...
            $query3 = "SELECT id FROM User WHERE name = ? and role= 'Admin'";      
            ...
            if($stmt1->rowCount()>0 && $result){
                ...
                {
                    $stmt3 = $this->connect()->prepare($query1);
                    if (!$stmt3->execute([$coursename])){
                        $stmt3=NULL;
                        header("location: /professor/manageCourses?error=failed");
                        exit();
                    }
                    header("location: /professor/manageCourses?error=professornotfound");
                    exit();
                }
            }
        }
